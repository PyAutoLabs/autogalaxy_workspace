...
`autogalaxy` by default (`pip install autogalaxy`) and model-fitting uses it automatically. This guide covers the
...
- **Bound Methods and the JIT Cache:** Why `jax.jit(galaxy.image_2d_from)` does not work, and the cache footgun.
...
this guide quotes no headline number.
...
traces it. You do *not* need `register_galaxies_classes` here — the galaxies are built inside the jitted function
...
The advanced path is JIT-ing library methods directly (`galaxy.image_2d_from`, `galaxies.image_2d_from`, etc.)
without going through a `Simulator` or `Analysis`.

Audience: users building custom forward models or scientific tools using PyAutoGalaxy primitives directly — not
...
"""
The recipes below need only a grid and a galaxy — no dataset files. Any galaxy works; this one is a simple
Sersic bulge.
"""
...
When a jitted function takes a `Galaxy` or `Galaxies` as an *argument*, JAX must flatten and unflatten that
object across the JIT boundary — i.e. the class must be registered as a JAX **pytree**. The library handles this
for you automatically in two situations:
...
After either, every `Galaxy`, `LightProfile`, `MassProfile` of the same class is JIT-safe for the rest of the
process. You never register individual classes yourself.

For a script like this one — no `Analysis` or `Simulator` in sight — the library provides a dedicated walker.
Call it once on representative galaxies, before the first jitted call:
"""
...
The single rule every recipe shares: when you decorate a function with `@jax.jit` that calls a PyAutoGalaxy
library method internally, **pass `xp=jnp` to that method inside the function body**. The `xp=jnp` tells the
library "you're inside a JAX trace — route array operations through `jax.numpy`".

Here is the pattern for JIT-ing `galaxy.image_2d_from`. Note two things: the `galaxy` is a traced argument
...
compiled trace — this is what makes parameter sweeps and `jax.vmap` over galaxies work naturally.
...
- **Chaining library calls:** grids and images *produced by* an `xp=jnp` call are marked as JAX-backed. Pass one
  to a second library call without `xp=jnp` and the library raises immediately:
...
In the recipe above the grid is closed over, never passed as a jitted argument. This is a hard rule, not a style
choice: `Grid2D` is **not a registered pytree** (`register_galaxies_classes` registers `Galaxy`, `Galaxies` and
profile classes — not grids). Pass a grid as an argument and JAX flattens it into an opaque leaf; the library
then fails inside the trace with errors like `AttributeError: DynamicJaxprTracer has no attribute over_sampled`:

```python
@jax.jit
def galaxy_image(galaxy, grid):                   # WRONG — grid as a traced argument
    return galaxy.image_2d_from(grid=grid, xp=jnp).array

galaxy_image(galaxy, grid)                        # AttributeError inside the trace
```

Passing the raw coordinate array instead (`galaxy_image(galaxy, grid.array)`) does not work either — library
methods need the full `Grid2D` (its mask and over-sampling), not a bare array of coordinates.

Closing over the grid costs you nothing in practice: in a parameter sweep or a fit, the grid is fixed while the
model varies — exactly the split between closure constants (grid) and traced arguments (galaxy) the recipe uses.
...
You may be tempted by the shorter bound-method form, JIT-ing the method itself so the galaxy rides along as
`self`:

```python
jitted = jax.jit(galaxy.image_2d_from)
...
1. **Register first** when a `Galaxy`/`Galaxies` is a jitted argument — one `register_galaxies_classes(galaxies)`
   call (or let an `Analysis`/`Simulator` with `use_jax=True` do it as a side effect).
2. **`@jax.jit` outside means `xp=jnp` inside** — on every library call in the function body.
3. **Grid closed over, model traced** — grids never cross the JIT boundary; galaxies do.
...
The wrapper types themselves (`ag.Array2D`, `ag.Grid2D`, ...) are **backend-polymorphic**: the same class holds a
...
image_eager = galaxies.image_2d_from(grid=grid, xp=jnp)
...
**Inside `@jax.jit`, the wrapper cannot cross the boundary outward.** Returning an `ag.Array2D` (or any autoarray
wrapper) from a jitted function is a hard error, always:

```python
@jax.jit
def galaxy_image_wrong(galaxy):
    return galaxy.image_2d_from(grid=grid, xp=jnp)    # returns the wrapper — TypeError
...
image_rewrapped = ag.Array2D(values=image, mask=grid.mask)
...
| `galaxies.image_2d_from(grid=grid, xp=jnp)` | `jax.Array` |
