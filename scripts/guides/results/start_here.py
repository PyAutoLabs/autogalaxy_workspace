...
This guide shows two complementary ways to get those results back into Python:
  ...
  - **Simple loading** — point at a single fit's `output/...` folder and load `.json` / `.fits` files directly with
    `from_json(...)` and `ag.Imaging.from_fits(...)`. The objects that come back behave exactly like the in-memory
    `Result` returned by `search.fit()`. This is the fastest way to inspect one fit, but everything you load sits
    in memory.
  - **Aggregator** — scrape a directory of completed fits and yield the same objects (`Galaxies`, `Samples`,
...
**FITS Files:** Load imaging products (data, fit, galaxy images) from the `image/` folder.
...
**Galaxies:** Producing images and computing quantities from the maximum likelihood `Galaxies`.
...
corresponds to a set of model parameters that were evaluated and accepted. This is the same `Samples` object that
...
like retrieving the images of each galaxy. It is the same `Galaxies` collection that the simple-loading section above
reached via `from_json(file_path="files/galaxies.json")`.
