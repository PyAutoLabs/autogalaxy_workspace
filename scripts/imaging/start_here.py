...
We focus on fitting a *galaxy-scale* target (a single galaxy). If your image contains additional
galaxies whose light blends with it, see the `imaging/features/extra_galaxies` examples. Group and
cluster scale systems are strong lensing analyses, covered by the `group` and `cluster` examples
of the `autolens_workspace`.
...
- **Model Fit:** Fitting the model to the data using the MultiStartProdigy non-linear search.
...
We now fit the data with the galaxy model using `MultiStartProdigy`, a multi-start gradient optimizer which finds
the best-fit galaxy model quickly.
...
Descending from a single starting point would frequently get stuck in a local maximum, because galaxy model
parameter spaces are complex and multi-modal. Running a wide population of starts in parallel is what makes a
gradient optimizer reliable here — the approach introduced for galaxy and lens modeling by GIGA-Lens (Gu, Huang
et al. 2022, arXiv:2202.07663).
...
between parameters. If the model says the effective radius is 1.6", this fit cannot tell you whether that is
1.6 +/- 0.01 or 1.6 +/- 0.5.

For most science you need those uncertainties. To get them, run
`autogalaxy_workspace/scripts/imaging/modeling.py`, which fits this same model with the nested sampling algorithm
`Nautilus` and returns the **full posterior** — every parameter's probability density, its errors, and the
correlations between parameters. It is slower, which is exactly why `start_here.py` uses the fast optimizer and
`modeling.py` does the statistically complete fit.
